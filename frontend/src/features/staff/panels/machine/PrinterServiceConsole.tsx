...
function ServiceActions({ request, onAction }: { request: MachineServiceRequest; onAction: (name: ServiceActionName) => void }) {
  if (request.status === "pending") return <><button className={focusRing} onClick={() => onAction("accept")}>Accept</button><button className={focusRing} onClick={() => onAction("reject")}>Reject</button></>;
  if (request.status === "accepted") return <button className={focusRing} onClick={() => onAction("start")}>Start</button>;
  if (request.status === "in_progress") return <><button className={focusRing} onClick={() => onAction("complete")}>Complete</button><button className={focusRing} onClick={() => onAction("fail")}>Fail</button></>;
  if (request.status === "completed") return <button className={focusRing} onClick={() => onAction("collect")}>Collect</button>;
  return request.status === "failed" ? <button className={focusRing} onClick={() => onAction("reprint")}>Reprint</button> : null;
}
...
function ActionForm({ action, values, setValues, printers, pools, pending, onCancel, onSubmit }: { action: ServiceActionName; values: ServiceDraft["actionValues"]; setValues: (values: ServiceDraft["actionValues"]) => void; printers: Machine[]; pools: PrinterPool[]; pending: boolean; onCancel: () => void; onSubmit: () => void }) {
  const input = (key: keyof typeof values, label: string) => <label className="grid gap-1 text-xs">{label}<input className={`desk-input ${focusRing}`} value={values[key]} onChange={(event) => setValues({ ...values, [key]: event.target.value })} /></label>;
  return <div className="mt-3 rounded-md border border-accent bg-bg p-3"><p className="font-semibold capitalize">{action}</p><div className="mt-2 grid gap-2 md:grid-cols-3">{action === "start" ? <><select aria-label="Printer" className={`desk-input ${focusRing}`} value={values.machine_id} onChange={(event) => setValues({ ...values, machine_id: event.target.value, consumable_pool_id: "" })}><option value="">Printer</option>{printers.map((printer) => <option key={printer.id} value={printer.id}>{printer.name}</option>)}</select><select aria-label="Pool" className={`desk-input ${focusRing}`} value={values.consumable_pool_id} onChange={(event) => setValues({ ...values, consumable_pool_id: event.target.value })}><option value="">Pool</option>{pools.map((pool) => <option key={pool.id} value={pool.id}>{pool.material} {pool.color}</option>)}</select>{input("estimated_minutes", "Estimated minutes")}{input("planned_quantity", "Planned grams")}</> : null}{action === "accept" ? <>{input("estimated_minutes", "Estimated minutes")}{input("planned_quantity", "Planned grams")}</> : null}{action === "complete" || action === "fail" ? <>{input("actual_minutes", "Actual minutes")}{input("actual_quantity", "Actual grams")}{action === "fail" ? <>{input("percent_complete", "Percent complete")}{input("reason", "Reason")}</> : null}</> : null}{action === "reject" ? input("reason", "Reason") : null}</div><div className="mt-2 flex gap-2"><button className={focusRing} disabled={pending} onClick={onSubmit}>Confirm</button><button className={focusRing} onClick={onCancel}>Cancel</button></div></div>;
}
