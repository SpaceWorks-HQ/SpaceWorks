    ...
    <div className="grid gap-4 border-t border-line p-3">
      ...
      <Subsection title="Manual usage">
        ...
        <div className="mt-3 grid gap-2">
          {manualUsage.data?.map((entry) => <p className="rounded-md border border-line p-2 text-sm" key={entry.id}>{entry.outcome} · {entry.consumed_quantity} {unitLabel} · {entry.duration_minutes} min</p>)}
        </div>
      </Subsection>
...
function ServiceActions({ request, onAction }: { request: MachineServiceRequest; onAction: (name: ServiceActionName) => void }) {
  ...
  return request.status === "completed" ? <button className={focusRing} onClick={() => onAction("collect")}>Collect</button> : null;
}
