...
function CheckInQr({ makerspaceId, registrationId }: { makerspaceId: number; registrationId: number }) {
  ...
  return <div className="mt-1">
    <button
      type="button"
      className="text-sm underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus"
      aria-expanded={open}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="desk-panel p-5"><h2 className="font-semibold text-ink">{title}</h2>{children}</section>; }
function Empty({ children = "Nothing to show yet." }: { children?: React.ReactNode }) { return <p className="mt-2 text-sm text-muted">{children}</p>; }
...
export function MemberActivityPanel({ activity, makerspaceId }: { activity: MemberActivity; makerspaceId: number }) {
  ...
  const requestRows = (rows: MachineServiceRequest[]) => rows.length ? <ul className="mt-3 space-y-2 text-sm text-muted">{rows.map((item) => <li key={`${item.title}-${item.status}`}><span className="font-medium text-ink">{item.title}</span> · {item.status}{item.queue_position ? ` · Queue ${item.queue_position}` : ""}</li>)}</ul> : <Empty />;
