...
- Guest Admins are **handout-only**: they issue accepted requests, create direct handouts, process scoped returns, and upload evidence, all through the same evidence/QR/remark/audit workflow as staff. They **cannot** accept/reject requests, edit inventory, or manage QR. The seeded Guest Admin role does include `ISSUE_DIRECT_LOAN` (`rbac._GUEST_ADMIN_ACTIONS`, pinned by `test_guest_admin_can_create_direct_loan`) — an older revision of this file said otherwise. Editing the Guest Admin role is allowed but capped: `role_services._validate_actions` refuses any grant outside `rbac.HANDOUT_ACTIONS`, so it can be narrowed and never widened.
