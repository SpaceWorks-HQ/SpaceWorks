...
def _read_dataset(dataset, tenant_id, closure, page_size, deadline, monotonic):
    ...
    queryset = _select_declared_relations(queryset, dataset)
    cursor_values, rows, count = None, [], 0
    while True:
        _check_deadline(dataset.path, count, deadline, monotonic)
        page_query = queryset
        if cursor_values is not None:
            page_query = page_query.filter(_after_keyset(dataset.keyset, cursor_values))
        page = list(page_query.order_by(*dataset.keyset)[:page_size])
        if not page:
            break
        dangling = _collect_user_ids(dataset, page, closure)
        _validate_dataset_integrity(dataset, page, tenant_id)
        rows.extend((row, dangling) for row in page)
        count += len(page)
        cursor_values = tuple(_key_value(page[-1], name) for name in dataset.keyset)
...
def _select_declared_relations(queryset, dataset):
    paths = {
...
def _after_keyset(names, values):
    result = Q()
...
def _key_value(row, name):
    field = next(
        field for field in row._meta.get_fields()
...
def _is_statement_timeout(exc):
    ...
    return getattr(cause, "sqlstate", None) == "57014" or "statement timeout" in str(exc).lower()
